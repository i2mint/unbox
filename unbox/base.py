...
class MyModuleGraph(ModuleGraph):
    def __init__(
        ...
        super().__init__()
        self._root = root
        self._rootpath = resolve_rootpath(root)
...
all_accessible_non_pkg_module_names = {
    x.name for x in all_accessible_modules if not x.ispkg
}
...
scanned_standard_lib_names = set(
    filter(is_importable, scan_locally_for_standard_lib_names(include_underscored=True))
)
